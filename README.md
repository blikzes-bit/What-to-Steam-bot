# «Во что?» — Steam Party Telegram Bot

MVP Telegram-бота для компаний друзей. Бот связывает Telegram с публичными Steam-профилями, находит общие игры участников, создаёт лобби с голосованием и присылает уведомления о скидках.

## Возможности

- привязка Steam по SteamID или ссылке на профиль;
- синхронизация публичной библиотеки и игрового времени;
- карточка профиля и статус участников группы;
- поиск общих игр;
- лобби: сбор участников → три игры → голосование → победитель;
- случайный выбор общей игры и проверка владельцев;
- неигранная библиотека, недавние игры и достижения;
- статистика библиотеки, уровень Steam и сравнение игроков;
- карточки игр, текущий онлайн, новости и проверка блокировок;
- каталог выгодных предложений и заметных новых релизов;
- настраиваемый ежедневный дайджест скидок и релизов;
- персональный список игр со скидочным порогом 25%, 50% или 75%;
- отдельный фоновый worker для проверки магазина.

## Команды

```text
/link <SteamID или ссылка>   привязать Steam
/sync                        обновить библиотеку
/profile                     показать свой профиль
/online                      статусы участников группы
/common                      общие игры группы
/lobby                       создать лобби
/random                      выбрать общую игру
/whoowns <AppID>             проверить владельцев в группе
/backlog                     показать неигранные игры
/stats                       статистика библиотеки
/level                       уровень Steam
/compare                     сравнить игроков ответом на сообщение
/recent                      недавние игры
/achievements <AppID>        достижения пользователя
/game <AppID>                информация об игре и онлайн
/news <AppID>                последние новости игры
/bans                        проверить блокировки
/deals <25|50|75>            текущие выгодные предложения
/releases                    заметные новые релизы
/digest all 50               включить ежедневный дайджест
/digest deals 75             дайджест только скидок
/digest releases             дайджест только релизов
/digest off                  отключить дайджест
/watch <AppID> <25|50|75>    следить за скидкой
/watchlist                   список уведомлений
/unwatch <AppID>             перестать следить
/privacy                     информация о данных
/unlink                      удалить связь со Steam
/delete_me                   полностью удалить свои данные
```

## Локальный запуск

Нужны Docker и Docker Compose.

1. Создайте бота через [@BotFather](https://t.me/BotFather).
2. Получите [Steam Web API key](https://steamcommunity.com/dev/apikey).
3. Скопируйте настройки:

```bash
cp .env.example .env
```

4. Заполните `BOT_TOKEN`, `STEAM_API_KEY` и задайте надёжный `POSTGRES_PASSWORD`. Пароль в `DATABASE_URL` должен совпадать.
   `DIGEST_HOUR_UTC` задаёт час ежедневной рассылки по UTC.
5. Запустите:

```bash
docker compose up -d --build
docker compose logs -f bot worker
```

Бот работает через long polling: домен, TLS-сертификат и открытый входящий порт не нужны.

## Развёртывание на Hetzner

Подойдёт обычный Ubuntu VPS с Docker Compose и минимум 1 GB RAM.

```bash
git clone <URL_РЕПОЗИТОРИЯ> vochto-bot
cd vochto-bot
cp .env.example .env
nano .env
docker compose up -d --build
docker compose ps
```

Обновление:

```bash
git pull --ff-only
docker compose up -d --build
```

PostgreSQL не публикуется наружу. Данные хранятся в Docker volume `postgres_data`.

## Надёжность и безопасность

- API-ключи загружаются только из окружения и не хранятся в Git;
- запросы к Steam ограничены по параллельности и повторяются после временных ошибок;
- Telegram-события защищены от частого повторения;
- неизвестные ошибки скрываются от пользователей и записываются в журнал;
- повторный запуск worker не создаёт параллельную проверку скидок;
- ежедневные дайджесты защищены от повторной отправки после перезапуска;
- удаление пользовательских данных подтверждается только в личном чате;
- редактирование сообщений лобби устойчиво к одновременным голосам.

## Архитектура

```text
bot     aiogram long polling и Telegram-команды
worker  периодическая проверка скидок
db      PostgreSQL
migrate Alembic перед запуском приложений
```

Это модульный монолит: оба процесса используют общий код и одну базу данных.

Каталог скидок и релизов изолирован в `SteamStoreClient`: витринные методы Steam Store не входят в стабильный Steamworks Web API и могут изменяться независимо от основных методов профиля.

## Лицензия

Проект распространяется по лицензии [MIT](LICENSE).

## Разработка без Docker

```bash
python -m venv .venv
source .venv/bin/activate
pip install -e . pytest pytest-asyncio respx ruff pyright
ruff check .
pytest
pyright
```

На Windows активация окружения: `.venv\Scripts\Activate.ps1`.
